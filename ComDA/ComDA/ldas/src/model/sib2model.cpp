...
void SiB2Model::config(const SiB2Config& cfg)
{
	...
	iter = 0;		//Add by liufeng 2019/3/14
}
...
//Only run one step %Add by liufeng 2019/3/14
int SiB2Model::runone();
{
	...
	if (iter == 0)
	{
		...
		// open the file containing forcing data
		m_iu(m_config.forcing_path.c_str());
	}
    ...
    // open/create the output file
    std::string fn=m_config.output_path+"out.txt";
    ofstream out(fn.c_str(), ios::app);
    fn=m_config.output_path+"out1.txt";
    ofstream out1(fn.c_str(), ios::app);
    fn=m_config.output_path+"out2.txt";
    ofstream out2(fn.c_str(), ios::app);
    fn=m_config.output_path+"out3.txt";
    ofstream out3(fn.c_str(), ios::app);
    fn=m_config.output_path+"out4.txt";
    ofstream out4(fn.c_str(), ios::app);

	const2();

	int nymd;
	driver(m_iu, isnow, nymd);
	balan(iplace = 1, nymd);
